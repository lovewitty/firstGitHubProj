<%@ Page Language="C#" AutoEventWireup="true" CodeFile="productTryManage.aspx.cs" Inherits="AdminManage_productTryManage" %>
...
<body>
  ...
  <TR height=28>
    <TD background=images/title_bg1.jpg>当前位置:查看产品试用记录</TD></TR>
  <TR>
        ...
        <ItemTemplate>
         <tr id="trIdrep" runat="server" onMouseOver="this.bgColor='#C4DFF7'" onMouseOut="this.bgColor='#ffffff'">
                <td nowrap><%#Container.ItemIndex+1%> </td>   
                    <td nowrap><%#Eval("ProductTitle")%></td> 
                <td nowrap><img src='../upload/ProductTry/<%#Eval("ProductPictures")%>' width=50px height=30px /><%#Eval("ProductPictures")%></td>
                <td nowrap><%#Eval("TotalCount")%></td>
                <td nowrap><%#Eval("LeftCount")%></td>
                   <td nowrap><%#Eval("currentBool")%></td>
