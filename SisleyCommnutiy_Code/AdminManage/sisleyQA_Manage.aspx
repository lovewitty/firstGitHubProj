<%@ Page Language="C#" AutoEventWireup="true" CodeFile="sisleyQA_Manage.aspx.cs" Inherits="AdminManage_sisleyQA_Manage" %>
...
<body>
  ...
  <TR height=28>
    <TD background=images/title_bg1.jpg>当前位置:查看Sisley-问答记录</TD></TR>
  <TR>
        ...
        <ItemTemplate>
         <tr id="trIdrep" runat="server" onMouseOver="this.bgColor='#C4DFF7'" onMouseOut="this.bgColor='#ffffff'">
                <td nowrap><%#Container.ItemIndex+1%> </td>   
                 <td nowrap><%#Cmn.Str.GetStr( Eval("ContentAnswer").ToString(),50,true)%></td>   
                  <td nowrap><%#Cmn.Str.GetStr(Eval("ContentQuestion").ToString(),50,true)%></td>   			
                  <td nowrap><%#Eval("SortNumb")%></td>
                  <td nowrap><%#Eval("QaTypeName")%></td>
                <td nowrap><%#Eval("QaSubTypeName")%></td>
                <td nowrap><%#Eval("DateAsk")%></td>
                   <td nowrap>
