<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SmartPerson_QA_Manage.aspx.cs" Inherits="AdminManage_SmartPerson_QA_Manage" %>
...
<body>
  ...
  <TR height=28>
    <TD background=images/title_bg1.jpg>当前位置:查看 贴身达人问题回答</TD></TR>
  <TR>
...
<TABLE cellSpacing=0 cellPadding=2 width="95%" align=center border=0>
  <TR>
    <TD align=left>
        ...
        <ItemTemplate>
         <tr id="trIdrep" runat="server" onMouseOver="this.bgColor='#C4DFF7'" onMouseOut="this.bgColor='#ffffff'">
                <td nowrap><%#Container.ItemIndex+1%> </td>   
                    <td nowrap><%#Eval("AskTypeName")%></td>  			 					
                <td nowrap><%#Cmn.Str.GetStr(Eval("QuestionContent").ToString(),80,true)%></td>  
                  <td nowrap><%#Eval("QuestionDate")%></td>             
                   <td nowrap><%#Eval("UserEmail")%></td>    
                      <td nowrap></td>      
                <td nowrap><%#Cmn.Str.GetStr(Eval("AnswerContent").ToString(),80,true)%></td> 
                      <td nowrap><%#Eval("DarenName")%></td>
                <td nowrap><%#Cmn.Str.GetStr(Eval("AnswerDate").ToString(), 9, false)%></td>
                <td nowrap><%#Eval("StateBool")%></td>
                <td nowrap>
        ...
        <FooterTemplate>
       <tr class="bgc_sum" >
                <td colspan="11">共<%=recordCount%>记录</td>                
              </tr>       
          </table></td>
        </tr>
      </table>
