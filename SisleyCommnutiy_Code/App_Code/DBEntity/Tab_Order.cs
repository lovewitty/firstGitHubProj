...
namespace DBEntity
{
	public partial class Tab_Order
	 ...
	 public int AddNew(Tab_Order model) 
 	 {
		string sql = "insert into Tab_Order(UserIdx_Fx,CounterNo,TotalPoints,OrderStatus,SourcesData,DateCreated)  values(@UserIdx_Fx,@CounterNo,@TotalPoints,@OrderStatus,@SourcesData,@DateCreated); select @@identity ;";
	 ...
	 public bool Update(Tab_Order model) 
 	 {
		 string sql = "update Tab_Order set UserIdx_Fx=@UserIdx_Fx,CounterNo=@CounterNo,TotalPoints=@TotalPoints,OrderStatus=@OrderStatus,SourcesData=@SourcesData,DateCreated=@DateCreated where Idx=@Idx";
	 ...
	 public bool Delete(string Idx) 
 	 {
		 int rows = SqlHelper.ExecuteNonQuery(CommandType.Text," delete from Tab_Order where Idx=@Idx",
			  new SqlParameter("Idx",Idx));
		 return rows > 0; 
 	 }

	private static Tab_Order ToModel(DataRow row)
	{
		Tab_Order model = new Tab_Order();
	...
	public Tab_Order Get(string Idx)
	{
		DataTable dt = SqlHelper.ExecuteDataset(CommandType.Text,"select * from Tab_Order  where Idx=@Idx",
		...
		Tab_Order model = ToModel(row);
		return model;
	}

	public IEnumerable<Tab_Order> ListAll()
	{
		List<Tab_Order> list = new List<Tab_Order>();
		DataTable dt = SqlHelper.ExecuteDataset(CommandType.Text,"select * from Tab_Order").Tables[0];
	...
	public IEnumerable<Tab_Order> ListAll(string strSql)
	{
		List<Tab_Order> list = new List<Tab_Order>();
	...
	public static DataSet GetDs_Where(string beginDate,string endDate,string keywords)
	{
		...
		string strSql = " select * from Tab_Order";
