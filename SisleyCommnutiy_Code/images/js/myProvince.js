...
{ "n": "新疆维吾尔自治区", "c": ["乌鲁木齐市", "克拉玛依市", "吐鲁番地区", "哈密地区", "和田地区", "阿克苏地区", "喀什地区", "克孜勒苏柯尔克孜自治州", "巴音郭楞蒙古自治州", "昌吉回族自治州", "博尔塔拉蒙古自治州", "伊犁哈萨克自治州", "塔城地区", "阿勒泰地区", "石河子市", "阿拉尔市", "图木舒克市", "五家渠市"] },
...
$.initCities = function (provEl, cityEl, defaultCity) {
    ...
    if (provEl.val() != '' && parseInt(provEl.val()) >= 0) {
        ...
        for (var i = 0; i < cities.length; i++) {
            cityHtml += '<option value="' + i + '"' + ((hasDefaultCity && (i == defaultCity)) ? ' selected="selected"' : '') + '>' + cities[i] + '</option>';
        }
...
* @param defaultCity
*/
$.initProvStr = function (prov, city, defaultProv, defaultCity) {
    var provEl = $(prov);
    var cityEl = $(city);
    var provHtml = '';
    ...
    for (var i = 0; i < $._cityInfo.length; i++) {
        if (i == defaultProv) {
            provHtml += $._cityInfo[i].n;
        }
    }
    provEl.html(provHtml);
    $.initCitiesStr(defaultProv, cityEl, defaultCity);
};
...
* @param defaultCity
*/
$.initCitiesStr = function (defaultProv, cityEl, defaultCity) {
    var cities = $._cityInfo[defaultProv].c; //此处要用页面端传进来的省份ID直接从省份数组中遍历城市
    var cityHtml = '';
    for (var i = 0; i < cities.length; i++) {
        if (i == defaultCity) {
            cityHtml += cities[i];
